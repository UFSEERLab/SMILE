...
my_b_season <- b.season(b0 = -16, b1 = 0.6, period = 3*52,t= 0:nweeks)
...
expand.grid(my_b0s, my_b1s, period) %>%  
  ...
  mutate(b = pmap(list(b0, b1, period), function(x,y,z) b.season(x,y,z*52, 0:nweeks))) %>%
  unnest(cols = c(b)) %>% 
...
vietnam_data %>% 
  ...
  filter(!my_category %in% c("vaccine_doses", "total_animal_cases", "total_pop_in_risk_area")) %>% 
